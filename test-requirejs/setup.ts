...
const allTemplatePaths: {[requirePath: string]: string} = {};
const TEST_REGEXP = /^\/test-requirejs\/[^\/]+\.[tj]s$/i;
const TEMPLATE_REGEXP = /^\/base\/(src|test-requirejs)\/.+\.html$/i;
Object.keys(__karma__.files).forEach(file => {
  if (TEST_REGEXP.test(file) && file !== '/base/test/setup.ts') {
    // Normalize paths to RequireJS module names.
    // If you require sub-dependencies of test files to be loaded as-is (requiring file extension)
    // then do not normalize the paths
    const normalizedTestModule = file.replace(/^\/base\/|\.ts$/g, '');
    allTestFiles.push(normalizedTestModule);
  } else if (TEMPLATE_REGEXP.test(file)) {
    const normalizedTemplate = file.replace(/^\/?base/, '/base');
...
require.config({
  ...
  callback: (pal: { initialize: () => void; }) => {
    ...
    started = true;
    pal.initialize();
    require(allTestFiles, () => __karma__.start());
  },
  paths: {
